 ...
 */
 ...
 (function() {
	...
	function createDropdownToggle() {
		...
		var icon = new DOMParser().parseFromString( gambitScreenReaderText.icon, 'text/html' ).body.firstElementChild;
		dropdownToggle.appendChild( icon);

		// Add screenreader text.
		var screenReaderText = document.createElement( 'span' );
		screenReaderText.classList.add( 'screen-reader-text' );
		screenReaderText.textContent = gambitScreenReaderText.expand;
	...
	function initNavigation( containerClass, naviClass ) {
		...
		(function() {
			...
			navigation.querySelectorAll( '.current-menu-ancestor > button' ).forEach( function( activeToggle ) {
				...
				activeToggle.querySelector( '.screen-reader-text' ).textContent = gambitScreenReaderText.collapse;
			});
