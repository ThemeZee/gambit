...
function gambit_gutenberg_support() {
	...
	add_theme_support( 'editor-color-palette', apply_filters( 'gambit_editor_color_palette_args', array(
		array(
...
function gambit_block_editor_assets() {
	...
	wp_enqueue_style( 'gambit-editor-styles', get_theme_file_uri( '/assets/css/gutenberg-styles.css' ), array(), '20210306', 'all' );

	// Enqueue Page Template Switcher Editor plugin.
	#wp_enqueue_script( 'gambit-page-template-switcher', get_theme_file_uri( '/assets/js/page-template-switcher.js' ), array( 'wp-blocks', 'wp-element', 'wp-edit-post' ), '20210306' );
}
 ...
 */
function gambit_block_editor_settings( $editor_settings ) {
...
#add_filter( 'block_editor_settings', 'gambit_block_editor_settings', 11 );


/**
 * Add body classes in Gutenberg Editor.
 */
function gambit_block_editor_body_classes( $classes ) {
	global $post;
	...
	if ( 'templates/template-fullwidth.php' === get_page_template_slug( $post->ID ) ) {
		$classes .= ' gambit-fullwidth-page-layout ';
	}
	...
	if ( 'templates/template-no-title.php' === get_page_template_slug( $post->ID ) or
		...
		$classes .= ' gambit-page-title-hidden ';
	}

	// Full-width / No Title Page Template?
	if ( 'templates/template-fullwidth-no-title.php' === get_page_template_slug( $post->ID ) ) {
		$classes .= ' gambit-fullwidth-page-layout gambit-page-title-hidden ';
	}

	return $classes;
}
#add_filter( 'admin_body_class', 'gambit_block_editor_body_classes' );
