...
class Gambit_Magazine_Posts_Grid_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget
		parent::__construct(
			'gambit-magazine-posts-grid', // ID
			sprintf( esc_html__( 'Magazine Posts: Grid (%s)', 'gambit' ), wp_get_theme()->Name ), // Name
			array( 
				'classname' => 'gambit_magazine_posts_grid', 
				'description' => esc_html__( 'Displays your posts from a selected category in a grid layout. Please use this widget ONLY in the Magazine Homepage widget area.', 'gambit' ) 
			) // Args
		);
	...
	function widget( $args, $instance ) {
		...
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'widget_gambit_magazine_posts_grid', 'widget' );
		}
		if ( ! is_array( $cache ) ) {
			$cache = array();
		}
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'widget_gambit_magazine_posts_grid', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	...
	function widget_title( $args, $settings ) {
		...
		if( !empty( $widget_title ) ) :
			...
			if( $settings['category'] > 0 ) : 
				...
				$link_title = sprintf( esc_html__( 'View all posts from category %s', 'gambit' ), get_cat_name( $settings['category'] ) );
				$link_url = esc_url( get_category_link( $settings['category'] ) );
				
				// Display Widget Title with link to category archive
				echo '<div class="widget-header">';
				echo '<h3 class="widget-title"><a class="category-archive-link" href="'. $link_url .'" title="'. $link_title . '">'. $widget_title . '</a></h2>';
				echo '</div>';
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id('layout'); ?>"><?php esc_html_e( 'Grid Layout:', 'gambit' ); ?></label><br/>
			<select id="<?php echo $this->get_field_id('layout'); ?>" name="<?php echo $this->get_field_name('layout'); ?>">
				<option <?php selected( $settings['layout'], 'two-columns' ); ?> value="two-columns" ><?php esc_html_e( 'Two Columns Grid', 'gambit' ); ?></option>
				<option <?php selected( $settings['layout'], 'three-columns' ); ?> value="three-columns" ><?php esc_html_e( 'Three Columns Grid', 'gambit' ); ?></option>
			</select>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_date' ); ?>">
				...
				<?php esc_html_e( 'Display post date', 'gambit' ); ?>
			</label>
		</p>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'meta_author' ); ?>">
				...
				<?php esc_html_e( 'Display post author', 'gambit' ); ?>
			</label>
		</p>
<?php
	} // form()
	...
	public function delete_widget_cache() {
		...
		wp_cache_delete( 'widget_gambit_magazine_posts_grid', 'widget' );
...
// Register Widget
add_action( 'widgets_init', 'gambit_register_magazine_posts_grid_widget' );

function gambit_register_magazine_posts_grid_widget() {

	register_widget( 'Gambit_Magazine_Posts_Grid_Widget' );
