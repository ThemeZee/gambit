 ...
 */
class Gambit_Magazine_Posts_Columns_Widget extends WP_Widget {
	...
	function __construct() {
		...
		// Setup Widget.
		parent::__construct(
			'gambit-magazine-posts-columns', // ID.
			esc_html__( 'Magazine (Columns)', 'gambit' ), // Name.
			array(
				'classname' => 'gambit-magazine-columns-widget',
	...
	function render( $args, $settings ) {
		...
		// Get cached post ids.
		$post_ids_category_one = gambit_get_magazine_post_ids( $this->id . '-left-category', $settings['category_one'], $settings['number'] );
		$post_ids_category_two = gambit_get_magazine_post_ids( $this->id . '-right-category', $settings['category_two'], $settings['number'] );
		?>
	...
	function category_title( $args, $settings, $category_id, $category_title ) {
		...
		if ( ! empty( $widget_title ) ) :
			...
			$widget_title = gambit_magazine_widget_title( $widget_title, $category_id );
	...
	function form( $instance ) {
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'category_one' ); ?>"><?php esc_html_e( 'Left Category:', 'gambit' ); ?></label><br/>
		...
		<p>
			<label for="<?php echo $this->get_field_id( 'highlight_post' ); ?>">
				...
				<?php esc_html_e( 'Highlight first post (big image + excerpt)', 'gambit' ); ?>
			</label>
		</p>
 ...
 * Register Widget
 */
function gambit_register_magazine_posts_columns_widget() {

	register_widget( 'Gambit_Magazine_Posts_Columns_Widget' );

}
add_action( 'widgets_init', 'gambit_register_magazine_posts_columns_widget' );
